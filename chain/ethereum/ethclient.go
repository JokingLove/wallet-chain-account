...
import (
	...
	"github.com/JokingLove/wallet-chain-account/common/global_const"
	"github.com/JokingLove/wallet-chain-account/common/helpers"
	"github.com/JokingLove/wallet-chain-account/common/retry"
...
func (c *clnt) BlockHeaderByRange(startHeight, endHeight *big.Int, chainId uint) ([]types.Header, error) {
...
func (c *clnt) BlockByNumber(number *big.Int) (*RpcBlock, error) {
	...
	var block *RpcBlock
	err := c.rpc.CallContext(ctxwt, &block, "eth_getBlockByHash", toBlockNumArg(number), true)
	if err != nil {
...
func (c *clnt) TxCountByAddress(address common.Address) (hexutil.Uint64, error) {
	...
	if err != nil {
		log.Error("Call eth_getTransactionCount fail", "err", err)
		return 0, err
	}
...
func (c *clnt) SuggestGasTipCap() (*big.Int, error) {
	...
	var hex hexutil.Big
	if err := c.rpc.CallContext(ctxwt, &hex, "eth_maxPriorityFeePerFas"); err != nil {
		return nil, err
	}
	return (*big.Int)(&hex), nil
}
...
func (c *clnt) FilterLogs(query ethereum.FilterQuery, chainId uint) (Logs, error) {
	...
	batchElems[0] = rpc.BatchElem{Method: "eth_getBlockByNumber", Args: []interface{}{toBlockNumArg(query.ToBlock)}}
	...
	if chainId == uint(global_const.ZkFairSepoliaChainId) ||
		chainId == uint(global_const.ZkFairChainId) {
		batchElems[0].Error = c.rpc.CallContext(ctxwt, &header, batchElems[0].Method, toBlockNumArg(query.ToBlock))
	...
	if batchElems[0].Error != nil {
		return Logs{}, fmt.Errorf("unable to query for the `FilterQuery#Block` header : %w", batchElems[0].Error)
	}
